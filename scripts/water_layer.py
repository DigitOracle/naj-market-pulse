...
def run(slug, do_push, tok):
    ...
    if not os.path.exists(src) or os.path.getsize(src) < 200: return None
    ...
    doc = {"district": slug, "generated": time.strftime("%Y-%m-%d"),
           "source": "Overture Maps water (sea, bay, lake, canal); swimming pools removed; clipped to the district and simplified to 6 m",
...
def main():
    ...
    slugs = want or sorted(os.path.basename(p)[:-len("_water.geojson")] for p in glob.glob(os.path.join(RAW, "*_water.geojson")))
