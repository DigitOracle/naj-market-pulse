...
out = {"updated": dt.date.today().isoformat(), "beds": BED_LABEL, "bands": BAND_LABEL, "developers": {},
    ...
    for name in seg["developers"]:
        ...
        if projects:
            rows = con.execute("select try_cast(TRANS_VALUE as double), try_cast(TRANS_VALUE as double)/nullif(try_cast(PROCEDURE_AREA as double),0), "
                               "IS_OFFPLAN_EN='Off-Plan', AREA_EN, ROOMS_EN from transactions where PROJECT_EN in (%s) and (USAGE_EN is null or USAGE_EN not like '%%Commercial%%') "
