# name-match only (no paid geocoding), all 40 districts
$ErrorActionPreference = "Continue"; $env:PYTHONIOENCODING = "utf-8"
Set-Location C:\Dev\naj-market-pulse
$log = "logs\refresh_free_$(Get-Date -Format yyyyMMdd_HHmm).log"
"start $(Get-Date -Format s)" | Out-File $log -Encoding utf8
& python scripts\bind_dld_buildings.py --no-geocode 2>&1 | Out-File $log -Append -Encoding utf8
& python scripts\bind_register_buildings.py --no-geocode 2>&1 | Out-File $log -Append -Encoding utf8
