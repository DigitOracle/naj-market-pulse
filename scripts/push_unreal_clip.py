...
slug, i, name, mp4, poster = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5]
title = sys.argv[6] if len(sys.argv) > 6 else "12 s sweep"
scope = "district" if i == "district" else "building"          # i = "district" publishes the district fly-through (map rail chip, v120)
i = i if scope == "district" else int(i)
key = f"unreal_{slug}_fly" if scope == "district" else f"unreal_{slug}_{i}"
...
items.append({"key": key, "kind": "unreal", "scope": scope, "district": slug, "i": i, "name": name, "title": title, "src": "/video/" + key, "poster": "/video/" + key + "?poster=1", "added": time.strftime("%Y-%m-%d")})
reg["items"] = items
