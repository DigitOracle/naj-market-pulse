...
## 13. Nationality goes external (17 Sep 2026)

Community nationality shares may now appear on a client-facing surface — a card, a post, a client link, a demo video. Kendall's decision, asked directly while building the Business Bay demo video: "nationality is okay if it's on a card or a post or anything like that... it's important demographic information." This replaces the 14 Sep rule that kept resident mix internal to him and Naj.

What the data is: the DEWA customer register, per community and move-in month. Business Bay reads Europe 24%, Arab world 22%, South Asia 21%, Russia & Central Asia 11%, Iran & Türkiye 8%, across 26,500 accounts with 18% carrying no nationality.

Two limits did not change, and neither is policy. **There is no building grain** — nationality is stored per community only, so "who lives in this tower" is a question the data cannot answer however the rule reads; a demo beat about Peninsula One has to become a beat about Business Bay. And **the size floors hold**: communities with 500+ accounts, groups at 5% or more, countries named only at 20+ accounts, whole percents, the remainder pooled. Those are disclosure control, not editorial caution — drop them and small groups become identifiable by subtraction.

One thing worth saying plainly because it will come up again: publishing who lives somewhere is not the same as offering it as a reason to live there. The first is demographic context. The second is the shape of unlawful steering in most jurisdictions, and it is also a claim this data cannot support, since an account is its holder rather than a household and a quarter of recent Business Bay accounts carry no nationality at all. Section 11 of `docs/GOV_DATA_METHODOLOGY.md` now carries the decision and both limits; bank Q040 moved from `by_rule` to `held` — publishable, but no client-facing screen shows it yet, because the RESIDENTS page is still behind the residents key. Q121 ("where are the buyers coming from?") moved to `needs_data`: it was never the same question, and buyer nationality is not in anything we hold.
